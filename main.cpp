...
// =======================================================
//  DDA Line + 8-Way Circle Drawing Algorithms
// =======================================================
...
// ---------- Raw pixel plot (NO glBegin/glEnd here) ----------
inline void plotPixelRaw(float x, float y)
{
    glVertex2f(x, y);
}
...
void drawLineDDA(float x1, float y1, float x2, float y2)
{
    ...
    glBegin(GL_POINTS);
    for (int i = 0; i <= (int)(steps * SCALE); i++)
    {
        glVertex2f(x, y);
        x += xInc / SCALE;
        y += yInc / SCALE;
...
// ---------- Thick DDA line helper ----------
void drawThickLineDDA(float x1, float y1, float x2, float y2, int thickness)
{
    float offset = 0.001f;

    if (fabs(x1 - x2) < 0.0001f) // vertical
    {
        float start = -((thickness - 1) / 2.0f) * offset;
        for (int i = 0; i < thickness; i++)
        {
            drawLineDDA(x1 + start + i * offset, y1, x2 + start + i * offset, y2);
        }
    }
    else if (fabs(y1 - y2) < 0.0001f) // horizontal
    {
        float start = -((thickness - 1) / 2.0f) * offset;
        for (int i = 0; i < thickness; i++)
        {
            drawLineDDA(x1, y1 + start + i * offset, x2, y2 + start + i * offset);
        }
    }
    else // diagonal/general
    {
        for (int i = 0; i < thickness; i++)
        {
            drawLineDDA(x1 + i * offset * 0.3f, y1, x2 + i * offset * 0.3f, y2);
        }
    }
}

// ---------- 8-way circle outline ----------
void drawCircle8Way(float xc, float yc, float r)
{
    ...
    while (x <= y)
    {
        plotPixelRaw(xc + (float)x / SCALE, yc + (float)y / SCALE);
        plotPixelRaw(xc - (float)x / SCALE, yc + (float)y / SCALE);
        plotPixelRaw(xc + (float)x / SCALE, yc - (float)y / SCALE);
        plotPixelRaw(xc - (float)x / SCALE, yc - (float)y / SCALE);
        plotPixelRaw(xc + (float)y / SCALE, yc + (float)x / SCALE);
        plotPixelRaw(xc - (float)y / SCALE, yc + (float)x / SCALE);
        plotPixelRaw(xc + (float)y / SCALE, yc - (float)x / SCALE);
        plotPixelRaw(xc - (float)y / SCALE, yc - (float)x / SCALE);
...
void drawFilledCircle8Way(float xc, float yc, float r)
{
    int radius = (int)(r * SCALE);
    int x = 0;
    int y = radius;
    ...
    while (x <= y)
    {
        ...
        {
            plotPixelRaw(xc + (float)xi / SCALE, yc + (float)y / SCALE);
            plotPixelRaw(xc + (float)xi / SCALE, yc - (float)y / SCALE);
        }

        for (int xi = -y; xi <= y; xi++)
        {
            plotPixelRaw(xc + (float)xi / SCALE, yc + (float)x / SCALE);
            plotPixelRaw(xc + (float)xi / SCALE, yc - (float)x / SCALE);
        }
...
void tree()
{
    ...
    drawThickLineDDA(-1.5f, 0.8f, -1.5f, 0.55f, 10);

    glColor3ub(0, 153, 51);
    drawFilledCircle8Way(-1.45f, 0.82f, 0.08f);
    drawFilledCircle8Way(-1.50f, 0.85f, 0.08f);
    drawFilledCircle8Way(-1.55f, 0.82f, 0.08f);
    ...
    glColor3ub(153, 51, 51);
    drawThickLineDDA(-0.8f, 0.8f, -0.8f, 0.57f, 10);

    glColor3ub(0, 153, 51);
    drawFilledCircle8Way(-0.85f, 0.82f, 0.08f);
    drawFilledCircle8Way(-0.80f, 0.85f, 0.08f);
    drawFilledCircle8Way(-0.75f, 0.82f, 0.08f);

    glColor3ub(153, 51, 51);
    drawThickLineDDA(1.55f, 0.65f, 1.55f, 0.8f, 10);

    glColor3ub(0, 153, 51);
    drawFilledTriangle(1.45f, 0.7f, 1.55f, 0.95f, 1.65f, 0.7f);

    glColor3ub(153, 51, 51);
    drawThickLineDDA(-0.2f, 0.65f, -0.2f, 0.8f, 10);

    glColor3ub(0, 153, 51);
    drawFilledTriangle(-0.3f, 0.7f, -0.2f, 0.95f, -0.1f, 0.7f);

    glColor3ub(153, 51, 51);
    drawThickLineDDA(1.9f, 0.7f, 1.9f, 0.85f, 10);

    glColor3ub(0, 153, 51);
    drawFilledTriangle(1.8f, 0.75f, 1.9f, 1.0f, 2.0f, 0.75f);
}
void shop() {
    // left shop start
...
void road_footpath() {
    ...
    // divider
    glColor3ub(255, 255, 255);
    drawThickLineDDA(-2.0f, 0.0f, -1.8f, 0.0f, 3);
    drawThickLineDDA(-1.7f, 0.0f, -1.5f, 0.0f, 3);
    drawThickLineDDA(-1.4f, 0.0f, -1.2f, 0.0f, 3);
    drawThickLineDDA(-1.1f, 0.0f, -0.9f, 0.0f, 3);
    drawThickLineDDA(-0.8f, 0.0f, -0.6f, 0.0f, 3);
    drawThickLineDDA(-0.5f, 0.0f, -0.3f, 0.0f, 3);
    drawThickLineDDA(-0.2f, 0.0f,  0.0f, 0.0f, 3);
    drawThickLineDDA( 1.7f, 0.0f,  1.9f, 0.0f, 3);
    drawThickLineDDA( 0.9f, 0.8f,  0.9f, 0.97f, 3);
    drawThickLineDDA( 0.9f,-0.97f, 0.9f,-0.8f, 3);

    // zebra-crossing start
    glColor3ub(255, 255, 255);
    drawThickLineDDA(0.2f,  0.3f, 0.2f, -0.3f, 5);
    drawThickLineDDA(0.4f, -0.3f, 0.4f,  0.3f, 5);
    drawThickLineDDA(0.6f,  0.5f, 1.2f,  0.5f, 5);
    drawThickLineDDA(0.6f,  0.7f, 1.2f,  0.7f, 5);
    drawThickLineDDA(1.4f,  0.3f, 1.4f, -0.3f, 5);
    drawThickLineDDA(1.6f,  0.3f, 1.6f, -0.3f, 5);
    drawThickLineDDA(0.6f, -0.5f, 1.2f, -0.5f, 5);
    drawThickLineDDA(0.6f, -0.7f, 1.2f, -0.7f, 5);

    // left cross
    drawThickLineDDA(0.2f,  0.2f, 0.4f,  0.2f, 10);
    drawThickLineDDA(0.2f,  0.1f, 0.4f,  0.1f, 10);
    drawThickLineDDA(0.2f,  0.0f, 0.4f,  0.0f, 10);
    drawThickLineDDA(0.2f, -0.1f, 0.4f, -0.1f, 10);
    drawThickLineDDA(0.2f, -0.2f, 0.4f, -0.2f, 10);

    // right cross
    drawThickLineDDA(1.4f,  0.2f, 1.6f,  0.2f, 10);
    drawThickLineDDA(1.4f,  0.1f, 1.6f,  0.1f, 10);
    drawThickLineDDA(1.4f,  0.0f, 1.6f,  0.0f, 10);
    drawThickLineDDA(1.4f, -0.1f, 1.6f, -0.1f, 10);
    drawThickLineDDA(1.4f, -0.2f, 1.6f, -0.2f, 10);

    // bottom cross
    drawThickLineDDA(0.7f, -0.5f, 0.7f, -0.7f, 10);
    drawThickLineDDA(0.8f, -0.5f, 0.8f, -0.7f, 10);
    drawThickLineDDA(0.9f, -0.5f, 0.9f, -0.7f, 10);
    drawThickLineDDA(1.0f, -0.5f, 1.0f, -0.7f, 10);
    drawThickLineDDA(1.1f, -0.5f, 1.1f, -0.7f, 10);
    ...
    drawThickLineDDA(-2.0f,  0.5f,  0.4f,  0.5f, 3);
    drawThickLineDDA( 0.4f,  0.5f,  0.4f,  1.0f, 3);
    drawThickLineDDA( 1.4f,  1.0f,  1.4f,  0.5f, 3);
    drawThickLineDDA( 1.4f,  0.5f,  2.0f,  0.5f, 3);
    drawThickLineDDA( 2.0f, -0.5f,  1.4f, -0.5f, 3);
    drawThickLineDDA( 1.4f, -0.5f,  1.4f, -1.0f, 3);
    drawThickLineDDA( 0.4f, -1.0f,  0.4f, -0.5f, 3);
    drawThickLineDDA( 0.4f, -0.5f, -2.0f, -0.5f, 3);

    glColor3ub(230, 230, 230);
    drawThickLineDDA(-2.0f,  0.3f,  0.6f,  0.3f, 3);
    drawThickLineDDA( 0.6f,  0.3f,  0.6f,  1.0f, 3);
    drawThickLineDDA( 1.2f,  1.0f,  1.2f,  0.3f, 3);
    drawThickLineDDA( 1.2f,  0.3f,  2.0f,  0.3f, 3);
    drawThickLineDDA( 2.0f, -0.3f,  1.2f, -0.3f, 3);
    drawThickLineDDA( 1.2f, -0.3f,  1.2f, -1.0f, 3);
    drawThickLineDDA( 0.6f, -1.0f,  0.6f, -0.3f, 3);
    drawThickLineDDA( 0.6f, -0.3f, -2.0f, -0.3f, 3);
}
...
void traffic_light1() {
    ...
    glColor3ub(153, 0, 0);
    drawThickLineDDA(0.55f, 0.4f, 0.7f, 0.4f, 5);
    drawThickLineDDA(0.7f, 0.4f, 0.7f, 0.15f, 5);
    ...
    glColor3ub(0, 0, 0);
    drawThickLineDDA(0.65f, 0.15f, 0.75f, 0.15f, 3);
    drawThickLineDDA(0.75f, 0.15f, 0.75f, -0.05f, 3);
    drawThickLineDDA(0.75f, -0.05f, 0.65f, -0.05f, 3);
    drawThickLineDDA(0.65f, -0.05f, 0.65f, 0.15f, 3);

    // side
    drawThickLineDDA(0.65f, 0.1f, 0.6f, 0.1f, 3);
    drawThickLineDDA(0.65f, 0.05f, 0.62f, 0.05f, 3);
    drawThickLineDDA(0.65f, 0.0f, 0.63f, 0.0f, 3);

    drawThickLineDDA(0.75f, 0.1f, 0.8f, 0.1f, 3);
    drawThickLineDDA(0.75f, 0.05f, 0.78f, 0.05f, 3);
    drawThickLineDDA(0.75f, 0.0f, 0.77f, 0.0f, 3);

    // bulbs
    glColor3ub(204, 0, 0);
    drawFilledCircle8Way(0.7f, 0.1f, 0.02f);

    glColor3ub(255, 204, 0);
    drawFilledCircle8Way(0.7f, 0.05f, 0.02f);

    glColor3ub(0, 128, 0);
    drawFilledCircle8Way(0.7f, 0.0f, 0.02f);
    // light end
}
...
void traffic_light2() {
    ...
    glColor3ub(153, 0, 0);
    drawThickLineDDA(1.3f, -0.35f, 1.3f, -0.2f, 5);
    drawThickLineDDA(1.3f, -0.2f, 1.0f, -0.2f, 5);
    ...
    glColor3ub(0, 0, 0);
    drawThickLineDDA(0.9f, -0.1f, 1.0f, -0.1f, 3);
    drawThickLineDDA(1.0f, -0.1f, 1.0f, -0.3f, 3);
    drawThickLineDDA(1.0f, -0.3f, 0.9f, -0.3f, 3);
    drawThickLineDDA(0.9f, -0.3f, 0.9f, -0.1f, 3);

    // side
    drawThickLineDDA(0.9f, -0.15f, 0.85f, -0.15f, 3);
    drawThickLineDDA(0.9f, -0.2f, 0.87f, -0.2f, 3);
    drawThickLineDDA(0.9f, -0.25f, 0.88f, -0.25f, 3);

    drawThickLineDDA(1.0f, -0.15f, 1.05f, -0.15f, 3);
    drawThickLineDDA(1.0f, -0.2f, 1.03f, -0.2f, 3);
    drawThickLineDDA(1.0f, -0.25f, 1.02f, -0.25f, 3);

    // bulbs
    glColor3ub(204, 0, 0);
    drawFilledCircle8Way(0.95f, -0.15f, 0.02f);

    glColor3ub(255, 204, 0);
    drawFilledCircle8Way(0.95f, -0.2f, 0.02f);

    glColor3ub(0, 128, 0);
    drawFilledCircle8Way(0.95f, -0.25f, 0.02f);
    // light end
}
...
void road_light() {
    // left-up
    glColor3ub(128, 128, 128);
    drawThickLineDDA(-1.9f, 0.4f, -1.8f, 0.4f, 3);
    drawThickLineDDA(-1.8f, 0.4f, -1.8f, 0.1f, 3);
    ...
    // left-down
    glColor3ub(128, 128, 128);
    drawThickLineDDA(-1.6f, -0.4f, -1.5f, -0.4f, 3);
    drawThickLineDDA(-1.5f, -0.4f, -1.5f, -0.1f, 3);
    ...
    // right-up
    glColor3ub(128, 128, 128);
    drawThickLineDDA(1.4f, 0.4f, 1.5f, 0.4f, 3);
    drawThickLineDDA(1.5f, 0.4f, 1.5f, 0.1f, 3);
    ...
    // right-down
    glColor3ub(128, 128, 128);
    drawThickLineDDA(1.7f, -0.4f, 1.8f, -0.4f, 3);
    drawThickLineDDA(1.8f, -0.4f, 1.8f, -0.1f, 3);
    ...
    // bottom-left
    glColor3ub(128, 128, 128);
    drawThickLineDDA(0.5f, -0.7f, 0.5f, -0.6f, 3);
    drawThickLineDDA(0.5f, -0.6f, 0.8f, -0.6f, 3);
    ...
    // bottom-right
    glColor3ub(128, 128, 128);
    drawThickLineDDA(1.3f, -1.0f, 1.3f, -0.9f, 3);
    drawThickLineDDA(1.3f, -0.9f, 1.0f, -0.9f, 3);
    ...
    // top-left
    glColor3ub(128, 128, 128);
    drawThickLineDDA(0.5f, 0.8f, 0.5f, 0.9f, 3);
    drawThickLineDDA(0.5f, 0.9f, 0.8f, 0.9f, 3);
    ...
    // top-right
    glColor3ub(128, 128, 128);
    drawThickLineDDA(1.3f, 0.5f, 1.3f, 0.6f, 3);
    drawThickLineDDA(1.3f, 0.6f, 1.0f, 0.6f, 3);
...
void boat1() {
    ...
    glColor3ub(255, 140, 26);
    drawFilledCircle8Way(0.1f, -0.8f, 0.05f);

    glColor3ub(179, 89, 0);
    drawFilledCircle8Way(-0.05f, -0.8f, 0.05f);
...
void boat2() {
    ...
    drawFilledCircle8Way(0.1f, -0.65f, 0.05f);

    glColor3ub(230, 230, 0);
    drawFilledCircle8Way(-0.05f, -0.65f, 0.05f);
...
void car1() {
    ...
    glColor3ub(230, 0, 0);
    drawLineDDA(0.07f, 0.24f, 0.03f, 0.21f);
    drawLineDDA(0.03f, 0.09f, 0.07f, 0.06f);
    drawLineDDA(-0.18f, 0.06f, -0.15f, 0.09f);
    drawLineDDA(-0.15f, 0.21f, -0.18f, 0.24f);
    drawLineDDA(-0.06f, 0.24f, -0.06f, 0.21f);
    drawLineDDA(-0.06f, 0.09f, -0.06f, 0.06f);
...
void car2() {
    ...
    drawLineDDA(0.07f, 0.24f, 0.03f, 0.21f);
    drawLineDDA(0.03f, 0.09f, 0.07f, 0.06f);
    drawLineDDA(-0.03f, 0.24f, -0.03f, 0.21f);
    drawLineDDA(-0.03f, 0.09f, -0.03f, 0.06f);

    glColor3ub(217, 217, 217);
    drawLineDDA(-0.06f, 0.25f, -0.06f, 0.05f);
    drawLineDDA(-0.06f, 0.05f, -0.5f, 0.05f);
    drawLineDDA(-0.5f, 0.05f, -0.5f, 0.25f);
    drawLineDDA(-0.5f, 0.25f, -0.06f, 0.25f);
...
void car3() {
    ...
    drawLineDDA(0.07f, 0.24f, 0.03f, 0.21f);
    drawLineDDA(0.03f, 0.09f, 0.07f, 0.06f);
    drawLineDDA(-0.13f, 0.06f, -0.1f, 0.09f);
    drawLineDDA(-0.1f, 0.21f, -0.13f, 0.24f);
    drawLineDDA(-0.03f, 0.24f, -0.03f, 0.21f);
    drawLineDDA(-0.03f, 0.09f, -0.03f, 0.06f);
...
void rain() {
    ...
    drawLineDDA(0.0f, 0.9f, 0.05f, 0.85f);
    drawLineDDA(0.0f, 0.7f, 0.05f, 0.65f);
    drawLineDDA(0.0f, 0.5f, 0.05f, 0.45f);
    drawLineDDA(0.0f, 0.3f, 0.05f, 0.25f);
    drawLineDDA(0.0f, 0.1f, 0.05f, 0.05f);

    drawLineDDA(0.05f, -0.9f, 0.0f, -0.85f);
    drawLineDDA(0.05f, -0.7f, 0.0f, -0.65f);
    drawLineDDA(0.05f, -0.5f, 0.0f, -0.45f);
    drawLineDDA(0.05f, -0.3f, 0.0f, -0.25f);
    drawLineDDA(0.05f, -0.1f, 0.0f, -0.05f);
...
void start() {
    ...
    glColor3ub(255, 255, 255);
    c="Traffic  Signal  In  Mirpur City";
...
int main(int argc, char** argv) {
    ...
    cout << "Project Title: Traffic Signal In Mirpur City \n" << endl;
    cout << "Group Member :" << endl;
    cout << "Shakil Uz Zaman \t\t 222-15-6184" << endl;
    cout << "Tanzid Ahmed Topu \t\t 222-15-6223" << endl;
    cout << "Mirza Anonno Probokta \t\t 222-15-6338" << endl;
